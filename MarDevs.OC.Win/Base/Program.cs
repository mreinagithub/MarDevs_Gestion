		...
		static void Main()
		{
			...
			using (SingleInstance spi = new SingleInstance(ruta))
			{
				...
				else
				{
					...
					if (MarDevs.OC.Win.Properties.Settings.Default.AppVersion != appVersion.ToString())
					{
						MarDevs.OC.Win.Properties.Settings.Default.Upgrade();
						MarDevs.OC.Win.Properties.Settings.Default.AppVersion = appVersionString;
					}
					...
					try
					{
						...
						DL.ConfigurarNHibernate("MarDevs.OC.Core", true);
						...
						if (ticket == null)
						{
							...
							ticket = miFormLogin.RealizarAutenticacion(MarDevs.OC.Win.Properties.Settings.Default.UltimoUsuarioLogueado);
						}
